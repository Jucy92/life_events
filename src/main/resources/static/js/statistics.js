    ...
    try {
            ...
            topContainer.innerHTML = top5.map((person, index) => `
                <div class="mb-2 pb-2 ${index < top5.length - 1 ? 'border-bottom' : ''}">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <strong>${person.giverName}</strong>
                            <small class="text-muted">(${person.relation || '미지정'})</small>
                        </div>
        ...
        if (data.length === 0) {
            ...
            tableBody.innerHTML = displayData.map(person => `
                <tr>
                    <td><strong>${person.giverName}</strong></td>
                    <td>${person.relation || '미지정'}</td>
                    <td>
                    ...
                    <td>
                        ${person.lastEventDate ? person.lastEventDate : '-'}<br>
                        <small class="text-muted">${person.lastEventType || ''}</small>
                    </td>
                </tr>
            `).join('');
