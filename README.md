...
- ✅ 회원가입 / 로그인 (JWT 인증)
- ✅ 이메일 인증 (가입 시 이메일 확인)
- ✅ 아이디 찾기 (이메일 기반 OTP 인증)
- ✅ 비밀번호 찾기 (이메일 기반 OTP 인증)
- ✅ 사용자별 데이터 격리 (멀티테넌트)
...
- ✅ **XSS 방어**: HTML 이스케이핑 적용
- ✅ **파일 업로드 검증**: MIME 타입 및 파일 크기 서버 측 검증
- ✅ **H2 콘솔**: 프로덕션 환경에서 비활성화
- ✅ **에러 메시지**: 민감한 정보 노출 방지
...
2. **이메일 설정**: SMTP 서버 정보 환경변수로 관리
3. **데이터베이스 비밀번호**: 최소 16자 이상의 복잡한 비밀번호 사용
4. **CORS**: 프로덕션에서는 실제 도메인만 허용
5. **HTTPS**: 프로덕션 배포 시 반드시 HTTPS 사용
6. **환경변수**: .env 파일을 절대 Git에 커밋하지 말 것
